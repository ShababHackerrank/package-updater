...
const packageVersionRegex = new RegExp('^\\d+\\.\\d+\\.\\d+$');
 ...
 * @param {boolean} [request.override] - Whether to apply the update as an override (used for transitive dependencies)
...
async function updateManifests(request, files) {
    for (const file of files) {
        ...
        if (request.override) {
            await applyResolutionManifestUpdate(request, file, manifest);
        } else {
            await applyDirectManifestUpdate(request, file, manifest);
        }
    }
...
/**
 * Update a resolution in a package.json file
 * @param {Object} request - The package update request
 * @param {string} file - Path to the package.json file
 * @param {Object} manifest - Parsed package.json content
 * @returns {Promise<void>}
 */
async function applyResolutionManifestUpdate(request, file, manifest) {
    const resolutions = getResolutions(manifest);

    if (!(request.packageName in resolutions)) {
        console.log(
            chalk.yellow(
                `Manifest resolution at ${file} does not contain the package ${request.packageName}`,
            ),
        );
    }

    const currentVersion = resolutions[request.packageName] ?? 'None';
    console.log(
        chalk.white(
            `Resolution version in file ${file} [current | update]: [${currentVersion} | ${request.newVersion}]`,
        ),
    );
    ...
    if (isVersionMatching(currentVersion, request.newVersion)) {
        console.log(
            chalk.yellow(
                `Manifest resolution at ${file} matched update version; skipping update...`,
            ),
        );
        return;
    }
 ...
 * @returns {Promise<void>}
 */
async function applyDirectManifestUpdate(request, file, manifest) {
...
/**
 * Gets resolutions as object from the manifest
 * @param {Object} manifest - Parsed package.json content
 * @returns {Object} - Object containing resolutions
 */
function getResolutions(manifest) {
...
/**
 * Applies a resolution to the package.json
 * @param {Object} command - Command object with update details
 * @returns {Promise<void>}
 */
async function applyResolution(command) {
    const {
        request: { packageName, apply, test, packageManager },
        version,
        file,
    ...
    console.log(
        chalk.green(
            `Updated ${packageName} to ${version} in resolutions of package.json of file ${file}`,
        ),
    );
...
/**
 * Runs `<package-manager> install` and/or `<package-manager> test` depending on the command options
 * @param {Object} command - Command object containing options
 * @param {boolean} command.apply - Option to install the package.json changes
 * @param {boolean} command.test - Option to run tests; Will install automatically
 * @param {string} command.file - File where the package.json is located
 * @param {string} command.packageManager - Package manager used: npm or yarn
 * @returns {Promise<void>}
 */
function applyAndTest(command) {
    const { apply, test, file, packageManager } = command;
