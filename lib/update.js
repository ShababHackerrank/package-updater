...
async function updateManifests(request, files) {
    for (const file of files) {
        ...
        if (request.override) {
            await applyOverrideManifestUpdate(request, file, manifest);
        } else {
...
/**
 * Update an override in a package.json file
 * @param {Object} request - The package update request
 * @param {string} file - Path to the package.json file
 * @param {Object} manifest - Parsed package.json content
 * @returns {Promise<void>}
 */
async function applyOverrideManifestUpdate(request, file, manifest) {
    if (!(request.packageName in manifest.overrides)) {
        console.log(
            chalk.yellow(
                `Manifest override at ${file} does not contain the package ${request.packageName}`,
            ),
        );
    }

    const overrides = getOverrides(manifest);
    const currentVersion = manifest.overrides[request.packageName] ?? 'None';
    console.log(
        chalk.white(
            `Override version in file ${file} [current | update]: [${currentVersion} | ${request.newVersion}]`,
        ),
    );
    ...
    if (isVersionMatching(currentVersion, request.newVersion)) {
        console.log(
            chalk.yellow(
                `Manifest override at ${file} matched update version; skipping update...`,
            ),
        );
        return;
    }
...
/**
 * Gets overrides as object from the manifest
 * @param {Object} manifest - Parsed package.json content
 * @returns {Object} - Object containing runtime and dev dependencies
 */
function getOverrides(manifest) {
    return {
...
/**
 * Applies an override to the package.json
 * @param {Object} command - Command object with update details
 * @returns {Promise<void>}
 */
async function applyOverride(command) {
    const {
        request: { packageName, apply, test, packageManager },
        version,
        file,
    ...
    console.log(
        chalk.green(
            `Updated ${packageName} to ${version} in overrides of package.json of file ${file}`,
        ),
    );
